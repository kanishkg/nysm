...
class model(object):
    ...
    def __init__(self, batch_size = 4, seq_length=20,rec_length=20,ckpt = False,output_dir = ''):
        ...
        with tf.variable_scope("temp_enc"):
            ...
            self.temp_enc = tf.map_fn(function,tf.transpose(self.input_tensor,[1,0,2,3,4,5]))
            self.temp_enc = tf.transpose(self.temp_enc,[1,0,2])

        with tf.variable_scope("fully_connected"):
            self.red_enc = tf.contrib.layers.fully_connected(self.temp_enc,4096)
            self.lstm_input = tf.concat([self.red_enc,self.input_record],axis = 2)

        with tf.variable_scope("LSTM"):
            self.cell = tf.nn.rnn_cell.LSTMCell(num_units=256, state_is_tuple=True)
            self.outputs, _ = tf.nn.dynamic_rnn(self.cell,self.lstm_input,dtype = tf.float32)

        with tf.variable_scope("output"):
            self.outputs = tf.contrib.layers.fully_connected(self.outputs,rec_length*2)
    ...
    def save_model(self):
	    saver.save(self.sess, self.output_dir+"model.ckpt")
    ...
    def def_loss(self, out, target):
        loss = tf.reduce_mean(tf.nn.l2_loss(target - out))
        return loss
    ...
    def forward(self, input_array,record_array, target_array):
        ...
        out = self.sess.run(self.outputs, {self.input_tensor : input_array, self.target : target_array})
