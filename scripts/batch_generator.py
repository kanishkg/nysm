...
class batch_generator:
    ...
    def __init__( self,batch_size = 8,istrain = True, input_file = '/scratch/kvg245/youtube_videos/video_dataset/video.h5',target_file = '/scratch/kvg245/youtube_videos/spherical_cordinate.npy',num_frames = 16,index_file = '/scratch/kvg245/youtube_videos/index.npy',seq_length = 20,rec_length = 20):
    ...
    def create_batch(self, data_list):
        ...
        record_batch = []
        target_batch = []
        ind = sorted([-3,-5,-8,-10,-12,-15,-17,-20,0,3,6,9,12,15,17,20])
        for data in data_list:
            ...
            for seq in range(self.seq_length):
                ...
                target.append(self.target_data[data[0]][data[1],[data[2]+seq*self.fps[data[0]] +int(i*self.fps[data[0]]/20.0) for i in range(20)], 2:])
                record.append(self.target_data[data[0]][data[1],[data[2]+seq*self.fps[data[0]] -int(i*self.fps[data[0]]/20.0)-1 for i in range(20)],2:])
                video.append(self.input_data[str(data[0])][[data[2]+i for i in ind],...])
        ...
        return {'input':input_batch, 'target':target_batch, 'record':record_batch }
...
if __name__ == "__main__":
    ...
    a = bg.get_batch_vec()
    print(a['input'].shape,a['target'].shape,a['record'].shape)
